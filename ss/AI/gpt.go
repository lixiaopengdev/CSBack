...
func InitGPT() {
	...
	openaiKey, openaiKeyExist := os.LookupEnv("OPENAI_KEY")
	if openaiKeyExist || len(openaiKey) > 0 {
...
func (gpt ChatGPT) Completions(msg []Messages) (resp Messages, err error) {
	...
	if err != nil {
		log.Println("GPT Completions Marshal error:", err)
		return resp, err
	}
	// log.Printf("request gtp json string : %v", string(requestData))
	req, err := http.NewRequest("POST", BASEURL+"chat/completions", bytes.NewBuffer(requestData))
	if err != nil {
		log.Println("GPT Completions NewRequestRequest error:", err.Error())
		return resp, err
	}
	...
	if err != nil {
		log.Println("GPT Completions Request Openai error:", response, err.Error())
		return resp, err
	}
	defer response.Body.Close()
	if response.StatusCode/2 != 100 {
		log.Println("GPT Completions Request Openai error_code:", response.Status)
		return resp, fmt.Errorf("gtp api %s", response.Status)
	}
	body, err := ioutil.ReadAll(response.Body)
	if err != nil {
		log.Println("GPT Completions Openai response error:", resp, err.Error())
		return resp, err
	}
	...
	if err != nil {
		log.Println("GPT Completions Openai response Unmarshal error:", resp, err.Error())
		return resp, err
	}
