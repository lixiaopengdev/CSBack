...
func (cq *CardQuery) QueryRule() *RuleQuery {
	query := &RuleQuery{config: cq.config}
...
func (cq *CardQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (cq *CardQuery) FirstIDX(ctx context.Context) uint64 {
...
func (cq *CardQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (cq *CardQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (cq *CardQuery) IDs(ctx context.Context) ([]uint64, error) {
	var ids []uint64
...
func (cq *CardQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CardQuery) WithRule(opts ...func(*RuleQuery)) *CardQuery {
	query := &RuleQuery{config: cq.config}
...
func (cq *CardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Card, error) {
	...
	if query := cq.withRule; query != nil {
		if err := cq.loadRule(ctx, query, nodes,
			func(n *Card) { n.Edges.Rule = []*Rule{} },
			func(n *Card, e *Rule) { n.Edges.Rule = append(n.Edges.Rule, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CardQuery) loadRule(ctx context.Context, query *RuleQuery, nodes []*Card, init func(*Card), assign func(*Card, *Rule)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Card)
	nids := make(map[uint64]map[*Card]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(card.RuleTable)
		s.Join(joinT).On(s.C(rule.FieldID), joinT.C(card.RulePrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(card.RulePrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(card.RulePrimaryKey[0]))
