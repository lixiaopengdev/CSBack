...
func (sq *SettingQuery) QueryOwner() *UserQuery {
...
func (sq *SettingQuery) WithOwner(opts ...func(*UserQuery)) *SettingQuery {
	query := &UserQuery{config: sq.config}
...
func (sq *SettingQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Setting, error) {
	...
	if query := sq.withOwner; query != nil {
		if err := sq.loadOwner(ctx, query, nodes, nil,
			func(n *Setting, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SettingQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Setting, init func(*Setting), assign func(*Setting, *User)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Setting)
	for i := range nodes {
		fk := nodes[i].UserID
