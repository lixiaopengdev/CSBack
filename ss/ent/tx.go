...
func (tx *Tx) init() {
	tx.Agora_token = NewAgora_tokenClient(tx.config)
	tx.Bundle = NewBundleClient(tx.config)
	tx.CSField = NewCSFieldClient(tx.config)
	...
	tx.Hidden = NewHiddenClient(tx.config)
	tx.Invite_Code = NewInvite_CodeClient(tx.config)
	tx.Join = NewJoinClient(tx.config)
	tx.Mask = NewMaskClient(tx.config)
	...
	tx.TimeDew = NewTimeDewClient(tx.config)
	tx.User = NewUserClient(tx.config)
	tx.User_auth = NewUser_authClient(tx.config)
	tx.User_history = NewUser_historyClient(tx.config)
}
...
// applies a query, for example: Agora_token.QueryXXX(), the query will be executed
